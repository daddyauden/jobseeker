...
class EmployeeController extends AdminBase
{
    ...
    public function searchAction()
    {
        ...
        $form = $this->createFormBuilder(null, array("attr" => array("role" => "form", "class" => "form")))
            ->setAction($this->generateUrl("employee_search"))
            ->setMethod("post")
        ...
        if ($this->isPost()) {
            ...
            if ($form->isValid() && $form->get("submit")->isClicked()) {
                ...
                $employee = $this->getRepo()->getByUser(array("email", $content['email']));
                if (null !== $employee) {
                    $uid = $employee["uid"]["uid"];
                    $experiences = $this->getRepo("UserBundle:Experience")->getExpBy(array("uid", $uid));
                    $educations = $this->getRepo("UserBundle:Education")->getEduBy(array("uid", $uid));
                    return $this->render("admin/employee/search.html.twig", array(
                    ...
                    $this->get("session")->getFlashBag()->add("danger", "Employee Not Exists, Please Change Another Email");
                    return $this->render('admin/employee/search.html.twig', array("form" => $form->createView()));
                }
            }
        }
    ...
    public function addAction()
    {
        ...
        $form = $this->createFormBuilder($user, array("attr" => array("role" => "form", "class" => "form")))
            ->setAction($this->generateUrl("employee_add"))
            ->setMethod("post")
            ->add('email', EmailType::class, array("label" => $this->trans("table.user.email"), "attr" => array("class" => "form-control input-sm")))
        ...
        if ($this->isPost()) {
            ...
            if ($form->isValid() && $form->get("save")->isClicked()) {
                ...
                if ($this->getRepo("UserBundle:User")->findOneBy(array("email" => $email))) {
                    ...
                    $this->getEm()->flush();
                    return $this->redirect($this->generateUrl("employee_add"));
                }
            }
        }

        return $this->render('admin/employee/add.html.twig', array("form" => $form->createView()));
    }
    ...
    public function listAction($page)
    {
        ...
        $pager = $this->generatePager("/admin/employee-list-%d", $page, $offset, $this->getRepo()->getTotal());
        return $this->render("admin/employee/list.html.twig", array("employees" => $this->getEmployee($page, $offset), "urls" => $pager->generateUrl()));
    }

    private function getEmployee($page = 1, $offset = 10)
    {
