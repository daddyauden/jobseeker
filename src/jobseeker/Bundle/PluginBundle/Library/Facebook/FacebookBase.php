...
abstract class FacebookBase
{
     ...
     */
    protected function getUserAccessToken($code = NULL)
    {
    ...
    public function getSignedRequest()
    {
        if (!$this->signedRequest) {
            if ($this->request->query->has('signed_request')) {
                $this->signedRequest = $this->parseSignedRequest($this->request->query->get('signed_request'));
            } else if ($this->cookie->get($this->getSignedRequestCookieName())) {
                $this->signedRequest = $this->parseSignedRequest($this->cookie->get($this->getSignedRequestCookieName()));
            }
        }
    ...
    public function getLoginUrl($params = array())
    {
        ...
        return $this->getUrl(
            ...
            array(
                'client_id' => $this->getAppId(),
                'redirect_uri' => isset($params["redirect_uri"]) ? $params["redirect_uri"] : $currentUrl, // possibly overwritten
                'state' => $this->state,
    ...
    protected function getCode()
    {
        if ($this->request->query->has('code')) {
            if ($this->state !== null && $this->request->query->has('code') && $this->state === $this->request->query->get('state')) {
                // CSRF state has done its job, so clear it
//                $this->state = null;
//                $this->clearPersistentData('state');
                return $this->request->query->get('code');
            } else {
    ...
    {
        if ($this->trustForwarded && $this->request->server->has('HTTP_X_FORWARDED_HOST')) {
            return $this->request->server->get('HTTP_X_FORWARDED_HOST');
        }
    ...
    {
        if ($this->trustForwarded && $this->request->server->has('HTTP_X_FORWARDED_PROTO')) {
        ...
        if ($this->request->server->has('HTTPS') && ($this->request->server->get('HTTPS') === 'on' || $this->request->server->get('HTTPS') == 1)) {
            return 'https';
        }
        /* nginx way of checking for https */
        if ($this->request->server->has('SERVER_PORT') && ($this->request->server->get('SERVER_PORT') === '443')) {
            return 'https';
        }
        return 'http';
    }
